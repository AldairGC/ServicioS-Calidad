...
class browser 
{
	...
	function isWebTV()
	{
		if(preg_match("webtv",$this->useragent))
		{
	...
	function isMSPIE()
	{
		if(preg_match("@mspie@i",$this->useragent) || preg_match("@pocket@i", $this->useragent))
		{
			...
			if (preg_match("mspie", $this->useragent))
				$this->version = $val[1];
			else {
	...
	function isMozStable()
	{
		if(preg_match("mozilla",$this->useragent) &&
		   ...
		   !preg_match("netscape",$this->useragent))
		{
			$this->browsertype = "Mozilla"; 
			$val = explode(" ",stristr($this->useragent,"rv:"));
	...
	function isLynx()
	{
		...
		{
			if (preg_match("amaya", $this->useragent))
			{
	...
	function isNetscape()
	{
		if(preg_match("netscape",$this->useragent))
		{
		...
		elseif(preg_match("mozilla",$this->useragent) && 
