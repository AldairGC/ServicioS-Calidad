...
if ($_GET["subir"]=="1")
{
	$uploaddir = "tmp7/";
	 ...
	 if(isset($_POST['boton']) && $error==UPLOAD_ERR_OK) { 
	   ...
	   rename("tmp7/".basename($_FILES['archivo']['name']), "tmp7/".$filename);
	   //echo "tmp7/".$filename;
	  } 
	   if($subido) { 
...
if ($_GET["borrar"]=="1")
{
	unlink("tmp7/".$_GET["nombre_archivo"]);
...
?>
                    ...
                    <form action="formato_productos7.php?id=<?= $_GET["id"] ?>&folio=<?= $mirow[0] ?>&subir=1" method="post" enctype="multipart/form-data" name="form"> 
