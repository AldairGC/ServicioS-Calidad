...
if ($_GET["subir"]=="1")
{
	$uploaddir = "tmp1/";
	 ...
	 if(isset($_POST['boton']) && $error==UPLOAD_ERR_OK) { 
	   ...
	   rename("tmp1/".basename($_FILES['archivo']['name']), "tmp1/".$filename);
	   //echo "tmp1/".$filename;
	  } 
	   if($subido) { 
...
if ($_GET["borrar"]=="1")
{
	unlink("tmp1/".$_GET["nombre_archivo"]);
...
?>
                    ...
                    <form action="formato_productos1.php?id=<?= $_GET["id"] ?>&folio=<?= $mirow[0] ?>&subir=1" method="post" enctype="multipart/form-data" name="form"> 
