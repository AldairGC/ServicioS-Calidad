...
if ($_GET["subir"]=="1")
{
	$uploaddir = "tmp/";
	 ...
	 if(isset($_POST['boton']) && $error==UPLOAD_ERR_OK) { 
	   ...
	   rename("tmp/".basename($_FILES['archivo']['name']), "tmp/".$filename);
	   //echo "tmp/".$filename;
	  } 
	   if($subido) { 
		
		echo "<font color=#324D8A>La evidencia fue cargada con exito..."; 
	   } else {
		echo "<font color=#324D8A>Se ha producido un error: ".$error;
  }
...
if ($_GET["borrar"]=="1")
{
	unlink("tmp/".$_GET["nombre_archivo"]);
...
?>
                    ...
                    <form action="formato_productos.php?id=<?= $_GET["id"] ?>&folio=<?= $mirow[0] ?>&subir=1" method="post" enctype="multipart/form-data" name="form"> 
