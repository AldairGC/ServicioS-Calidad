...
if ($_GET["subir"]=="1")
{
	$uploaddir = "tmp6/";
	 ...
	 if(isset($_POST['boton']) && $error==UPLOAD_ERR_OK) { 
	   ...
	   rename("tmp6/".basename($_FILES['archivo']['name']), "tmp6/".$filename);
	   //echo "tmp6/".$filename;
	  } 
	   if($subido) { 
...
if ($_GET["borrar"]=="1")
{
	unlink("tmp6/".$_GET["nombre_archivo"]);
...
?>
                    ...
                    <form action="formato_productos6.php?id=<?= $_GET["id"] ?>&folio=<?= $mirow[0] ?>&subir=1" method="post" enctype="multipart/form-data" name="form"> 
